...
import { apiUrl, assetUrl, getAuthHeaders } from '../lib/api'

// The gallery lives on the server so every visitor and every admin machine
// sees the same photos. It is no longer mirrored into localStorage, because a
// per-browser copy made the gallery look different on each computer.
...
async function fetchRemoteGallery() {
  try {
    const data = await requestJson(apiUrl('/api/gallery'))

    if (!data || !Array.isArray(data.items)) {
      throw new Error('Unexpected response from the gallery API.')
    }
    ...
    // Show the built-in photos so the public page still looks complete, but
    // make the failure visible instead of silently serving this browser's
    // own cached copy.
    console.error('Unable to reach the gallery API.', error)
...
async function checkSession() {
  ...
  // Ask the backend whether this token is actually still valid. Previously the
  // app assumed "a token exists in this browser" meant "signed in", which made
  // the admin state depend on the individual machine instead of the server.
  try {
    await requestJson(apiUrl('/api/admin/me'), {
...
async function requestPasswordReset() {
  ...
  try {
    const response = await requestJson(apiUrl('/api/admin/forgot-password'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function updatePasswordAfterRecovery() {
  ...
  try {
    const response = await requestJson(apiUrl('/api/admin/reset-password'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function submitPhoto() {
  ...
  // Saving must go through the backend. Writing to localStorage instead only
  // changed the gallery inside one browser on one machine, which looked like a
  // successful save but was invisible to every other visitor.
  if (!isAuthenticated.value) {
    notice.value = 'Please sign in as admin before saving gallery changes.'
    return
  }
  ...
  try {
    ...
    const url = form.value.id ? apiUrl(`/api/gallery/${form.value.id}`) : apiUrl('/api/gallery')
    ...
    if (!response) {
      throw new Error('The server did not confirm the save. Please try again.')
    }
...
async function deletePhoto(id) {
  if (!isAuthenticated.value) {
    notice.value = 'Please sign in as admin before deleting gallery items.'
    return
  }

  try {
    await requestJson(apiUrl(`/api/gallery/${id}`), {
      method: 'DELETE',
...
<template>
  <section id="gallery" class="gallery">
    <div class="container">
      ...
      <p v-if="backendUnreachable" class="backend-warning">
        Showing placeholder photos — the website could not reach the server.
        Admin sign-in and saving are unavailable until the backend is online.
      </p>

      <div class="gallery-grid">
        <figure v-for="photo in photos" :key="photo.id" class="gallery-item">
          <img :src="assetUrl(photo.image)" :alt="photo.title" loading="lazy" />
        ...
        <div v-else class="admin-content">
          ...
          <form class="admin-form" @submit.prevent="submitPhoto">
            ...
            <div v-if="form.image" class="preview-box">
              <img :src="assetUrl(form.image)" :alt="form.title || 'Preview image'" />
            </div>
          ...
          <div class="gallery-list-box">
            ...
            <ul class="gallery-list">
              <li v-for="photo in photos" :key="photo.id">
                <img :src="assetUrl(photo.image)" :alt="photo.title" />
    ...
    <div v-if="selectedPhoto" class="lightbox" role="dialog" aria-modal="true" :aria-label="selectedPhoto.title" @click.self="closePhoto">
      <div class="lightbox-content">
        ...
        <img :src="assetUrl(selectedPhoto.image)" :alt="selectedPhoto.title" />
